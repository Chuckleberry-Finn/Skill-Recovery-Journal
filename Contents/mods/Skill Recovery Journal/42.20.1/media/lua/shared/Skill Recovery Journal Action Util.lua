...
function SRJ.handleIdentity(player, JMD)
	...
	if protections == 1 and (isClient() or isServer()) then
...
function SRJ.checkStaticConditions(player, JMD, doReading)
	...
	if doReading and (not JMD or not JMD["author"]) then
		return false, "IGUI_PlayerText_NothingWritten"

	elseif player:hasTrait(CharacterTrait.ILLITERATE) and (SandboxVars.SkillRecoveryJournal.IlliterateSpeedMultiplier or 0) <= 0 then
...
function SRJ.processJournalTick(self, player, JMD, doReading)
    ...
    if #recipeList > 0 then
        ...
        if getDebug() and self.updates % 20 == 0 then
            print("SRJ RECIPE DEBUG: remaining=" .. #recipeList .. " chunk=" .. tostring(chunk)
                .. " interval=" .. tostring(self.durationData.recipeInterval)
                .. " updates=" .. tostring(self.updates) .. "/" .. tostring(self.durationData.intervals))
        end
    ...
    self.durationData = self.durationData or {}
    self.durationData.rates = self.durationData.rates or {}
    self.durationData.flatRates = self.durationData.flatRates or {}

    if getDebug() and self.updates % 20 == 0 then
        print("SRJ IDENTITY DEBUG: JMD=" .. tostring(JMD) .. " JMD.gainedXP=" .. tostring(JMD.gainedXP)
            .. " ledgerID=" .. tostring(self.item:getModData()["SRJ"] and self.item:getModData()["SRJ"].ledgerID)
        ...
        for perkID, perkXP in pairs(processXpMap) do
            ...
            if perk and SRJ.bSkillValid(perk) then
                ...
                if not doReading then
                    ...
                    if getDebug() and self.updates % 20 == 0 then
                        print("SRJ WRITE DEBUG " .. perkID .. ": journalXP=" .. tostring(JMD.gainedXP[perkID])
                            .. " target=" .. tostring(gained) .. " rate=" .. tostring(rate)
                            .. " readXP=" .. tostring(currentXP)
                            .. " updates=" .. tostring(self.updates) .. "/" .. tostring(self.durationData.intervals))
                    end
                    ...
                    if currentXP < perkXP then
                        ...
                        if rate and rate > 0 and currentXP + rate > perkXP then
                            rate = math.max(perkXP - currentXP, 0.001)
                        end

                        if rate and rate > 0 and oneTimeUse then
                            local used = JMD.usedXP[perkID] or 0
                            local remaining = math.max((JMD.gainedXP[perkID] or 0) - used, 0)
                            if remaining <= 0 then
                                rate = false
                            elseif rate > remaining then
                                rate = remaining
                            end
                        end
        ...
        for perkID, journalFlatXP in pairs(JMD.flatGainedXP) do
            ...
            if perk and SRJ.bSkillValid(perk) then
                ...
                if currentFlatXP < journalFlatXP then
                    ...
                    if flatRate and flatRate > 0 and currentFlatXP + flatRate > journalFlatXP then
                        flatRate = math.max(journalFlatXP - currentFlatXP, 0.001)
                    end

                    if flatRate and flatRate > 0 and oneTimeUse then
                        local used = JMD.usedXP.flat[perkID] or 0
                        local remaining = math.max(journalFlatXP - used, 0)
                        if remaining <= 0 then
                            flatRate = false
                        elseif flatRate > remaining then
                            flatRate = remaining
                        end
                    end
    ...
    if killsEnabled and not self.killsComplete then
        ...
        if getDebug() and self.updates % 20 == 0 then
            print("SRJ KILLS DEBUG: gainedZ=" .. tostring(zombies) .. " gainedS=" .. tostring(survivors)
                .. " journalZ=" .. tostring(JMD.kills.Zombie) .. " journalS=" .. tostring(JMD.kills.Survivor)
                .. " zRate=" .. tostring(self.durationData.rates.zKills) .. " sRate=" .. tostring(self.durationData.rates.sKills)
                .. " updates=" .. tostring(self.updates) .. "/" .. tostring(self.durationData.intervals))
        end
    ...
    if doReading then
        if not self.modDataReadComplete then
            ...
            local data = SRJ.modDataHandler.copyDataToPlayer(player, JMD)
        ...
        if not self.modDataStoredComplete then
            ...
            local data = SRJ.modDataHandler.copyDataToJournal(player, JMD)
            if data and #data > 0 then
                for _, id in pairs(data) do
                    table.insert(self.changesBeingMade, id)
                end
    ...
    if not changesMade then
        ...
        if SRJ.isAuthoritative() then
            SRJ.ledger.syncDisplay(self.item, player)

            -- serverStop/complete). Throttled to avoid a disk write every tick.
            if self.updates % 30 == 0 then
                SRJ.ledger.save(player)
            end
        end
    end
