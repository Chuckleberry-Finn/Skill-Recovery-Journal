...
function SRJ.calculateGainedKills(journalModData, player, doReading)
    ...
    if doReading then
		local readXP = SRJ.ledger.getReadXPAny(player)
        ...
        accountedZombieKills = (journalModData.kills and journalModData.kills.Zombie or 0)
        accountedSurvivorKills = (journalModData.kills and journalModData.kills.Survivor or 0)
    end
...
function SRJ.calculateReadWriteRates(player, item, timeFactor, gainedRecipes, gainedSkills, flatGainedSkills, doReading, updateInterval)
    ...
    local journalModData = SRJ.isAuthoritative()
        and SRJ.ledger.getJournalRecord(item, player)
        or SRJ.modDataHandler.getDisplayData(item)
    local readXP = SRJ.ledger.getReadXPAny(player)
	local storedJournalXP = journalModData["gainedXP"] or {}

	local actionTimeMulti
	if doReading then
		actionTimeMulti = SandboxVars.SkillRecoveryJournal.ReadTimeSpeed or 1
	else
		actionTimeMulti = SandboxVars.SkillRecoveryJournal.TranscribeSpeed or 1
	end

	if player:hasTrait(CharacterTrait.ILLITERATE) then
		actionTimeMulti = actionTimeMulti * math.max(SandboxVars.SkillRecoveryJournal.IlliterateSpeedMultiplier or 0, 0.001)
	end
	...
	if SRJ.modDataHandler.hasModDataToTransfer(player, journalModData, doReading) then
    ...
    if doReading then
        ...
        local usedXP = journalModData.usedXP or {}
        ...
        for perkID, journalXP in pairs(storedJournalXP) do
            if Perks[perkID] and validSkills[perkID] then
                ...
                if oneTimeUse then
                    -- the journal's own pool is a second, independent limit
                    local remaining = math.max(journalXP - (usedXP[perkID] or 0), 0)
                    currentlyReadXP = math.max(currentlyReadXP, journalXP - remaining)
                end
        ...
        local readFlatXP = readXP.flat or {}
        local usedFlatXP = usedXP.flat or {}
        for perkID, journalFlatXP in pairs(storedFlatJournalXP) do
            ...
            if perk and SRJ.bSkillValid(perk) then
                ...
                if oneTimeUse then
                    local remaining = math.max(journalFlatXP - (usedFlatXP[perkID] or 0), 0)
                    currentlyReadFlatXP = math.max(currentlyReadFlatXP, journalFlatXP - remaining)
                end
