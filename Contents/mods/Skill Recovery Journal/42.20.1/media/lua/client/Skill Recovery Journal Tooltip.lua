...
local function SRJ_generateTooltip(JMD, player, itemObj)
	local blankJournalTooltip = getText("IGUI_Tooltip_Empty").."\n"

	if not JMD or not JMD["author"] then return blankJournalTooltip, "", false end

	local storedJournalXP = JMD["gainedXP"]
	if not storedJournalXP then return blankJournalTooltip, "", false end
	...
	for perkID,xp in pairs(storedJournalXP) do
		...
		if perk then
			...
			if show then
				local journalXP = xp
				local usedXP = JMD.usedXP
				if oneTimeUse and usedXP and usedXP[perkID] then
					journalXP = math.max(0, journalXP - usedXP[perkID])
				end
	...
	if SandboxVars.SkillRecoveryJournal.RecoverRecipes == true then
		local recipeNum = JMD["learnedRecipeCount"] or 0
	...
	local srj = itemObj and itemObj:getModData()["SRJ"]
	local usernameInsert = (srj and srj.owner) and " ("..srj.owner..")" or ""
...
function ISToolTipInv:render()
	if not ISContextMenu.instance or not ISContextMenu.instance.visibleCheck then
		...
		if itemObj and player and instanceof(itemObj, "InventoryItem") and itemObj:getType() == "SkillRecoveryBoundJournal" then
			...
			local journalModData = SRJ.modDataHandler.getDisplayData(itemObj)

			local tooltipStart, skillsRecord, warning = SRJ_generateTooltip(journalModData, player, itemObj)
