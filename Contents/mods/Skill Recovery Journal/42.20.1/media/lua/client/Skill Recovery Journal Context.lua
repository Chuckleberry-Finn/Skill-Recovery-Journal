	...
	for i,item in ipairs(actualItems) do
		...
		if item:getType() == "SkillRecoveryBoundJournal" then
			...
			if player and player.getSteamID then
				...
				local display = srj and srj.display
				local pSteamID = player:getSteamID()
				if (not display) or (not display.author) then
					emptyBook = true
				elseif pSteamID ~= 0 then
					local journalID = display.identity or {}
			...
			local illiterate = player:hasTrait(CharacterTrait.ILLITERATE) and (SandboxVars.SkillRecoveryJournal.IlliterateSpeedMultiplier or 0) <= 0
